...
public function buscara (Request $req){
  $crit=$req['criterio'];
  $consulta =\DB::select("SELECT idu,archivo,nombre,apellido,tipo,correo,password,activo FROM admins WHERE nombre LIKE '%$crit%' or apellido LIKE '%$crit%' or tipo  LIKE '%$crit%' or correo  LIKE '%$crit%'");
  return view("Reporteadmin",["consulta"=>$consulta, 'crit'=> $crit]);
}
