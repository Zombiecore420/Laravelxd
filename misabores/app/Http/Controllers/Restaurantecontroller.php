  ...
  public function Guardaedicionrestaurante (Request $Request)
{
  ...
  public function buscarr (Request $req){
    $crit=$req['criterio'];
    $consulta =\DB::select("SELECT Clave_restaurante, nombre, tel, calle, Ncalle, colonia, municipo,
                            cp, correo, estado 
                            FROM restaurantes 
                            WHERE 
                            nombre LIKE '%$crit%' or tel LIKE '%$crit%' or calle LIKE '%$crit%' or colonia LIKE '%$crit%' or municipo LIKE '%$crit%' or cp LIKE '%$crit%' or correo LIKE '%$crit%' or estado LIKE '%$crit%' ");
    return view("Reporterestaurante",["consulta"=>$consulta, 'crit'=> $crit]);
  }
  }
